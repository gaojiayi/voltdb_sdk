...
import com.gaojy.cache.voltdb.core.dbsource.DataLoaderImpl;
import com.gaojy.cache.voltdb.core.dbsource.opt.IDataLoader;

/**
 * VoltBulkLoader is meant to run for long periods of time. Multiple threads can operate on a single instance of the
 * VoltBulkLoader to feed and bulk load a single table. It is also possible for multiple instances of the VoltBulkLoader
 * to operate concurrently on the same table or different tables as long as they share the same Client instance. All
 * instances of VoltBulkLoader using a common Client share a pool of threads dedicated to feeding individual partitions.
 * There is also a thread dedicated to processing multi-partition tables. Finally, each instance of VoltBulkLoader has a
 * thread, dedicated to processing failed bulk inserts. If multiple threads are inserting rows into the same
 * VoltBulkLoader instance and batch inserts are relatively frequent, using multiple VoltBulkLoaders should improve
 * performance characteristics particularly if one thread is injecting more failed rows than other threads. If one
 * thread is injecting too many failures, relative to other threads operating on the same table, that thread should be
 * isolated a different process. If failed rows are a common problem, throughput can also be improved by using multiple
 * instances of VoltBulkLoader.
 */
public class VoltBulkLoader
{
	...
	private final ScheduledThreadPoolExecutor m_ses = CoreUtils.getScheduledThreadPoolExecutor("Periodic-Flush", 1,
	...
	private JDBCLoader jdbc;

	private Connection conn;

	// Constructor allocated through the Client to ensure consistency of VoltBulkLoaderGlobals
	public VoltBulkLoader(ClientImpl clientimpl, JDBCLoader jdbc, String tableName, int maxBatchSize) throws Exception
	{
		this(clientimpl, jdbc, tableName, maxBatchSize, false);
	}

	public VoltBulkLoader(ClientImpl clientimpl, JDBCLoader jdbc, String tableName, int maxBatchSize, boolean upsertMode)
			throws Exception
	{
		...
		if (m_upsert)
		{
			...
			while (pkeyInfo.advanceRow())
			{
				...
				{
					pKeyName = pkeyInfo.getString("COLUMN_NAME");
					hasPkey = true;
					break;
				}
			}
			if (!hasPkey)
			{
				...
				throw new IllegalArgumentException(String.format(
						"The --update argument cannot be used because the table %s does not have a primary key. "
								+ "Either remove the --update argument or add a primary key to the table.", tableName));
			}
		}
		...
		idataLoader = new DataLoaderImpl(jdbc, this);
	...
	public Map<String, Integer> getPkey()
	{
		return new HashMap<String, Integer>()
		{
			private static final long serialVersionUID = -3867175826659586269L;

			{
				if (null != pKeyName)
				{
					for (Map.Entry<Integer, String> en : m_colNames.entrySet())
					{
						if (pKeyName.equals(en.getValue()))
						{
							put(pKeyName, en.getKey());
							break;
						}
					}
				}
	 ...
	 * @param seconds
	 *            Interval in seconds, passing <code>seconds <= 0</code> value will cancel periodic flush
	 */
	...
	void generateError(Object rowHandle, Object[] objectList, String errMessage)
	{
		...
		jdbc.handleError((RowWithMetaData) rowHandle, dummyResponse, dummyResponse.getStatusString());
	 ...
	 *
	 * @param rowHandle
	 *            User supplied object used to distinguish failed insert attempts
	 * @param fieldList
	 *            List of fields associated with a single row insertion
	...
	public void insertRow(Object rowHandle, Object... fieldList) throws InterruptedException
	{
		...
		if (fieldList.length != m_columnCnt)
		{
			String errMsg;
			if (rowHandle == null)
				errMsg = "Error: insertRow received incorrect number of columns; " + fieldList.length + " found, "
						+ m_columnCnt + " expected";
			else
				errMsg = "Error: insertRow received incorrect number of columns; " + fieldList.length + " found, "
						+ m_columnCnt + " expected for row: " + rowHandle.toString();
			generateError(rowHandle, fieldList, errMsg);
			return;
		}

		idataLoader.insertRow((RowWithMetaData) rowHandle, fieldList);
	...
	{
		idataLoader.updateBatchSize();
	}

	/**
	 * Called to asynchronously force the VoltBulkLoader to submit all the partially full batches in all partitions of
	 * the table to the Client for insert. To wait for all rows to be processed, use drain(). This method will also
	 * flush pending rows submitted by other VoltBulkLoader instances working on the same table and using the same
	 * instance of Client.
	 */
	...
	/**
	 * Called to synchronously force the VoltBulkLoader to submit all the partially full batches in all partitions of
	 * the table to the Client for insert. This call will wait until all previously submitted rows (including retried
	 * failed batch rows) have been processed and received responses from the Client.
	 * 
	 * @throws java.lang.InterruptedException
	 */
	public synchronized void drain() throws InterruptedException
	{
		...
		{
			/*
			 * try { //m_partitionTable[i].flushAllTableQueues().get(); } catch (ExecutionException e) {
	...
	/**
	 * Waits for all pending inserts to be acknowledged and then closes this instance of the VoltBulkLoader. During and
	 * after the invocation of close(), calls to insertRow will get an Exception. All other instances of VoltBulkLoader
	...
	/**
	 * As other instances of VoltBulkLoader working on the same table could alter the size of batches, this method
	 * provides the means to examine the current batch size.
	 *
	 * @return The size of batches currently being submitted for this table
	 */
	public int getMaxBatchSize()
	{
		return (m_maxBatchSize == 0 ? 1 : m_maxBatchSize);
	}

	/**
	 * @return The number rows that have been received by this instance of VoltBulkLoader but have not been processed by
	 *         the Client
	 */
	...
	/**
	 * @return The number of rows that have been received by this instance of VoltBulkLoader and processed by the Client
	 *         including failed inserts that have performed callbacks using BulkLoaderFailureCallBack.
	 */
